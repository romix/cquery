...
static void getDocAndHover(const optional<std::string>& hover,
  ...
  auto hoverString = hover ? *hover : detailed_name;
  sections.push_back(CommentSection(hoverString));
}
    ...
    case SymbolKind::Type: {
      ...
      if (type.def)
        getDocAndHover(type.def->hover, type.def->comments,
                       type.def->detailed_name, sections);
      break;
    }
    case SymbolKind::Func: {
      ...
      if (func.def) {
        getDocAndHover(func.def->hover, getCommentOrOverriddenComment(db, func),
                       func.def->detailed_name, sections);
      }
      break;
    }
    case SymbolKind::Var: {
      ...
      if (var.def)
        getDocAndHover(var.def->hover, var.def->comments,
                       var.def->detailed_name, sections);
      break;
    }
  ...
  void Run(Ipc_TextDocumentHover* request) override {
    ...
    for (const SymbolRef& ref :
      ...
      if (!sections.empty()) {
        ...
        // Last section is the actual hover containing a signature.
        if (!sections.back().description.empty()) {
          out.result->contents.push_back(lsMarkedString());
          out.result->contents.back().value =
              *sections.back().description.begin();
          out.result->contents.back().language = file->def->language;
        }
        ...
        for (size_t idx = 0, e = sections.size() - 1; idx != e; ++idx) {
          ...
          if (!section.description.empty()) {
            out.result->contents.push_back(lsMarkedString());
            out.result->contents.back().value = *section.description.begin();
            out.result->contents.back().language = "text";
          }
        }
      }
